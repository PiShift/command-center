<x-layouts.app title="Bank Accounts">
    <div x-data="bankAccountTransfers({
        accounts: @js($accounts->map(fn($account) => [
            ...
            'usd_exchange_rate' => (float) ($account->effective_usd_exchange_rate ?? 40),
        ])),
    ...
    <div
        ...
        class="fixed inset-0 z-50 flex items-start sm:items-center justify-center px-3 sm:px-4 py-4 overflow-y-auto"
    >
        <div class="absolute inset-0 bg-black/45" @click="close()"></div>

        <div class="relative w-full max-w-lg max-h-[95vh] overflow-y-auto rounded-2xl border border-line bg-card shadow-[0_20px_60px_rgba(0,0,0,0.18)]">
            ...
            <form method="POST" action="{{ route('bank-accounts.transfer.store') }}" class="p-5 space-y-4">
                ...
                <div x-show="isCrossCurrency" x-cloak class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="exchange_rate" type="number" step="0.000001" min="0.000001" :value="exchangeRate" @input="updateField('exchangeRate', $event.target.value)" :required="isCrossCurrency" :disabled="!isCrossCurrency" class="w-full rounded-lg border border-line bg-surface px-3 py-2 text-[14px] text-ink focus:border-accent focus:bg-card focus:outline-none transition-colors" placeholder="40">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.05em] text-muted">Date</label>
                ...
                <div class="pt-1 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2">
                    <button type="button" @click="close()" class="w-full sm:w-auto px-4 py-2 rounded-md border border-line bg-surface hover:bg-hairline text-[13px] font-medium text-ink transition-colors cursor-pointer">Cancel</button>
                    <button type="submit" class="w-full sm:w-auto px-4 py-2 rounded-md bg-accent hover:bg-accent-hover text-[13px] font-medium text-white transition-colors cursor-pointer">Transfer Funds</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function bankAccountTransfers(config) {
            return {
                ...
                selectedUsdRate() {
                    const candidate = [this.fromAccount, this.toAccount].find((account) => {
                        return String(account?.currency || '').toUpperCase() === 'USD';
                    });

                    const value = Number(candidate?.usd_exchange_rate || 40);
                ...
                handleAccountChange() {
                    ...
                    this.rateDefault = this.selectedUsdRate();
                    this.exchangeRate = this.rateDefault;
                    this.lastEdited = { amountSent: 0, amountReceived: 0, exchangeRate: 0 };
                    this.recalculateCrossCurrency();
                },
